...
![配图：StateGraph示意——四个节点围绕共享状态，蓝色箭头读状态，橙色箭头写状态](/assets/17-langgraph-intro-01-stategraph.png)
- 每个步骤是一个 **Node（节点）**
- 步骤之间的跳转是 **Edge（边）**
- 所有共享数据存在 **State（状态）** 里
...
![配图：条件分支图——Agent到决策圆，是到工具并循环，否到输出](/assets/17-langgraph-intro-02-conditional-edges.png)
