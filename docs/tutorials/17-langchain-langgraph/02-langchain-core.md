...
![配图：左侧LLM标注无状态被红线划掉，右侧LLM+Memory组合，小黑作为记忆管理员向LLM注入历史消息](/assets/17-langchain-core-01-memory-inject.png)
...
![配图：RAG四步流水线——加载→切分→向量化→检索，小黑在检索端等待](/assets/17-langchain-core-02-rag-pipeline.png)
...
![配图：思考→行动→观察三个节点围成循环，小黑在中间旋转](/assets/17-langchain-core-03-agent-loop.png)
