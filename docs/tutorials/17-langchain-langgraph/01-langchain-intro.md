...
![配图：左侧小黑被乱线缠绕（手写胶水代码），右侧小黑冷静拼接积木（组件化组合）](/assets/17-langchain-intro-01-glue-vs-compose.png)
...
![配图：LCEL管道系统，小黑在拧水管接头，数据从输入流经Prompt→LLM→Parser](/assets/17-langchain-intro-02-lcel-pipe.png)
...
![配图：小黑在机器上拧下一个齿轮（GPT-4），换上另一个（Claude），其他部件不动](/assets/17-langchain-intro-03-swappable-parts.png)
